...
/**
 * An in-memory implementation of {@link BaseSessionService} assuming {@link Session}
 * objects are mutable regarding their state map, events list, and last update time.
 *
 * <p>
 * This implementation stores sessions, user state, and app state directly in memory using
 * concurrent maps for basic thread safety. It is suitable for testing or single-node
 * deployments where persistence is not required.
 *
 * <p>
 * Note: State merging (app/user state prefixed with {@code _app_} / {@code _user_})
 * occurs during retrieval operations ({@code getSession}, {@code createSession}).
 */
public final class InMemorySessionService implements BaseSessionService {
	...
	private final ConcurrentMap<String, ConcurrentMap<String, ConcurrentMap<String, Session>>> sessions;

	// Structure: appName -> userId -> stateKey -> stateValue
	private final ConcurrentMap<String, ConcurrentMap<String, ConcurrentMap<String, Object>>> userState;
	...
	public Single<Session> createSession(String appName, String userId, @Nullable ConcurrentMap<String, Object> state,
		...
		ConcurrentMap<String, Object> initialState = (state == null) ? new ConcurrentHashMap<>()
		...
		Session newSession = Session.builder(resolvedSessionId)
			.appName(appName)
			.userId(userId)
			.state(initialState)
	...
	public Maybe<Session> getSession(String appName, String userId, String sessionId,
		...
		Session storedSession = sessions.getOrDefault(appName, new ConcurrentHashMap<>())
		...
		if (config.numRecentEvents().isEmpty() && config.afterTimestamp().isPresent()) {
			...
			eventsInCopy.removeIf(event -> getEventTimestampEpochSeconds(event) < threshold.getEpochSecond());
		}
	...
	public Single<ListSessionsResponse> listSessions(String appName, String userId) {
		...
		Map<String, Session> userSessionsMap = sessions.getOrDefault(appName, new ConcurrentHashMap<>()).get(userId);
		...
		List<Session> sessionCopies = userSessionsMap.values()
			.stream()
	...
	public Completable deleteSession(String appName, String userId, String sessionId) {
		...
		ConcurrentMap<String, Session> userSessionsMap = sessions.getOrDefault(appName, new ConcurrentHashMap<>())
			.get(userId);
	...
	public Single<Event> appendEvent(Session session, Event event) {
			...
			if (stateDelta != null && !stateDelta.isEmpty()) {
				stateDelta.forEach((key, value) -> {
					if (key.startsWith(State.APP_PREFIX)) {
						...
						appState.computeIfAbsent(appName, k -> new ConcurrentHashMap<>()).put(appStateKey, value);
					}
		...
		sessions.getOrDefault(appName, new ConcurrentHashMap<>())
			.getOrDefault(userId, new ConcurrentHashMap<>())
	...
	/**
	 * Creates a shallow copy of the session, but with deep copies of the mutable state
	 * map and events list. Assumes Session provides necessary getters and a suitable
	 * constructor/setters.
	...
	/**
	 * Creates a copy of the session containing only metadata fields (ID, appName, userId,
	 * timestamp). State and Events are explicitly *not* copied.
	...
	/**
	 * Merges the app-specific and user-specific state into the provided *mutable*
	 * session's state map.
	...
	private Session mergeWithGlobalState(String appName, String userId, Session session) {
		...
		appState.getOrDefault(appName, new ConcurrentHashMap<String, Object>())
			.forEach((key, value) -> sessionState.put(State.APP_PREFIX + key, value));

		userState.getOrDefault(appName, new ConcurrentHashMap<>())
			.getOrDefault(userId, new ConcurrentHashMap<>())
