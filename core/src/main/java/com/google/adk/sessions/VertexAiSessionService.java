...
public final class VertexAiSessionService implements BaseSessionService {
	...
	/**
	 * Creates a new instance of the Vertex AI Session Service with a custom ApiClient for
	 * testing.
	 */
	...
	/**
	 * Creates a session service with specified project, location, credentials, and HTTP
	 * options.
	 */
	public VertexAiSessionService(String project, String location, Optional<GoogleCredentials> credentials,
			Optional<HttpOptions> httpOptions) {
		this.client = new VertexAiClient(project, location, credentials, httpOptions);
	}

	@Override
	public Single<Session> createSession(String appName, String userId, @Nullable ConcurrentMap<String, Object> state,
			@Nullable String sessionId) {

		String reasoningEngineId = parseReasoningEngineId(appName);
		return client.createSession(reasoningEngineId, userId, state)
			.map(getSessionResponseMap -> parseSession(getSessionResponseMap, appName, userId, sessionId))
			.toSingle();
	}

	private static Session parseSession(JsonNode getSessionResponseMap, String appName, String userId,
			String fallbackSessionId) {
		String sessId = Optional.ofNullable(getSessionResponseMap.get("name"))
		...
		if (getSessionResponseMap != null && getSessionResponseMap.has("sessionState")) {
			...
			if (sessionStateNode != null) {
				sessionState = objectMapper.convertValue(sessionStateNode,
	...
	public Single<ListSessionsResponse> listSessions(String appName, String userId) {
		...
		return client.listSessions(reasoningEngineId, userId)
			.map(listSessionsResponseMap -> parseListSessionsResponse(listSessionsResponseMap, appName, userId))
			.defaultIfEmpty(ListSessionsResponse.builder().build());
	}

	private ListSessionsResponse parseListSessionsResponse(JsonNode listSessionsResponseMap, String appName,
			String userId) {
		List<Map<String, Object>> apiSessions = objectMapper.convertValue(listSessionsResponseMap.get("sessions"),
				new TypeReference<List<Map<String, Object>>>() {
				});

		List<Session> sessions = new ArrayList<>();
		for (Map<String, Object> apiSession : apiSessions) {
			String sessionId = Iterables.getLast(Splitter.on('/').splitToList((String) apiSession.get("name")));
			Instant updateTimestamp = Instant.parse((String) apiSession.get("updateTime"));
			Session session = Session.builder(sessionId)
				.appName(appName)
				.userId(userId)
				.state(apiSession.get("sessionState") == null ? new ConcurrentHashMap<>() : objectMapper
					.convertValue(apiSession.get("sessionState"), new TypeReference<ConcurrentMap<String, Object>>() {
					}))
				.lastUpdateTime(updateTimestamp)
				.build();
	...
	public Single<ListEventsResponse> listEvents(String appName, String userId, String sessionId) {
		String reasoningEngineId = parseReasoningEngineId(appName);
		return client.listEvents(reasoningEngineId, sessionId)
	...
	private ListEventsResponse parseListEventsResponse(JsonNode listEventsResponse) {
		...
		return ListEventsResponse.builder()
			.events(objectMapper
				.convertValue(sessionEventsNode, new TypeReference<List<ConcurrentMap<String, Object>>>() {
				})
				.stream()
	...
	public Maybe<Session> getSession(String appName, String userId, String sessionId,
		...
		String reasoningEngineId = parseReasoningEngineId(appName);
		return client.getSession(reasoningEngineId, sessionId).flatMap(getSessionResponseMap -> {
			String sessId = Optional.ofNullable(getSessionResponseMap.get("name"))
				.map(name -> Iterables.getLast(Splitter.on('/').splitToList(name.asText())))
				.orElse(sessionId);
			Instant updateTimestamp = Optional.ofNullable(getSessionResponseMap.get("updateTime"))
				.map(updateTime -> Instant.parse(updateTime.asText()))
				.orElse(null);

			ConcurrentMap<String, Object> sessionState = new ConcurrentHashMap<>();
			if (getSessionResponseMap != null && getSessionResponseMap.has("sessionState")) {
				sessionState.putAll(objectMapper.convertValue(getSessionResponseMap.get("sessionState"),
						new TypeReference<ConcurrentMap<String, Object>>() {
						}));
			}

			return listEvents(appName, userId, sessionId).map(response -> {
				Session.Builder sessionBuilder = Session.builder(sessId)
					.appName(appName)
					.userId(userId)
					.lastUpdateTime(updateTimestamp)
					.state(sessionState);
	...
	private static List<Event> filterEvents(List<Event> originalEvents, @Nullable Instant updateTimestamp,
			Optional<GetSessionConfig> config) {
		List<Event> events = originalEvents.stream()
			.filter(event -> updateTimestamp == null
	...
	public Single<Event> appendEvent(Session session, Event event) {
		...
		return BaseSessionService.super.appendEvent(session, event).flatMap(
				e -> client.appendEvent(reasoningEngineId, session.id(), SessionJsonConverter.convertEventToJson(e))
					.toSingleDefault(e));
	}
	...
	static String parseReasoningEngineId(String appName) {
		...
		if (!matcher.matches()) {
			throw new IllegalArgumentException("App name " + appName + " is not valid. It should either be the full"
	...
	private static final Pattern APP_NAME_PATTERN = Pattern
		.compile("^projects/([a-zA-Z0-9-_]+)/locations/([a-zA-Z0-9-_]+)/reasoningEngines/(\\d+)$");
