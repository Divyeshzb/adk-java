...
abstract class ApiClient {
	...
	ApiClient(Optional<String> apiKey, Optional<HttpOptions> customHttpOptions) {
		...
		try {
			...
			throw new IllegalArgumentException(
					"API key must either be provided or set in the environment variable" + " GOOGLE_API_KEY.", e);
		}
	...
	ApiClient(Optional<String> project, Optional<String> location, Optional<GoogleCredentials> credentials,
		...
		try {
			...
			throw new IllegalArgumentException(
					"Project must either be provided or set in the environment variable" + " GOOGLE_CLOUD_PROJECT.", e);
		}
	...
	private void applyHttpOptions(HttpOptions httpOptionsToApply) {
		...
		if (httpOptionsToApply.headers().isPresent()) {
			ImmutableMap<String, String> mergedHeaders = ImmutableMap.<String, String>builder()
	...
	static HttpOptions defaultHttpOptions(boolean vertexAI, Optional<String> location) {
		...
		HttpOptions.Builder defaultHttpOptionsBuilder = HttpOptions.builder().headers(defaultHeaders.buildOrThrow());

		if (vertexAI && location.isPresent()) {
			defaultHttpOptionsBuilder
				.baseUrl(Ascii.equalsIgnoreCase(location.get(), "global") ? "https://aiplatform.googleapis.com"
	...
	GoogleCredentials defaultCredentials() {
		try {
			...
			throw new GenAiIOException(
					"Failed to get application default credentials, please explicitly provide credentials.", e);
		}
	}
