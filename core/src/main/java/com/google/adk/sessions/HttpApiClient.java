...
public class HttpApiClient extends ApiClient {
	...
	HttpApiClient(Optional<String> project, Optional<String> location, Optional<GoogleCredentials> credentials,
	...
	public ApiResponse request(String httpMethod, String path, String requestJson) {
		boolean queryBaseModel = Ascii.equalsIgnoreCase(httpMethod, "GET")
				&& path.startsWith("publishers/google/models/");
		...
		String requestUrl = String.format("%s/%s/%s", httpOptions.baseUrl().get(), httpOptions.apiVersion().get(),
				path);
	...
	private void setHeaders(Request.Builder requestBuilder) {
		for (Map.Entry<String, String> header : httpOptions.headers().orElse(ImmutableMap.of()).entrySet()) {
		...
		if (apiKey.isPresent()) {
			...
			GoogleCredentials cred = credentials
				.orElseThrow(() -> new IllegalStateException("credentials is required"));
			try {
			...
			try {
				...
				if (e.getMessage()
					.contains("because the return value of"
