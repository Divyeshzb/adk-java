...
final class SessionJsonConverter {
	...
	static String convertEventToJson(Event event) {
		...
		metadataJson.put("long_running_tool_ids",
		...
		eventJson.put("timestamp", new HashMap<>(
				ImmutableMap.of("seconds", event.timestamp() / 1000, "nanos", (event.timestamp() % 1000) * 1000000)));
	...
	private static Content convertMapToContent(Object rawContentValue) {
		...
		if (rawContentValue instanceof Map) {
			...
			logger.warn("Unexpected type for 'content' in apiEvent: {}", rawContentValue.getClass().getName());
			return null;
		}
	}
	...
	static Event fromApiEvent(Map<String, Object> apiEvent) {
		...
		if (apiEvent.get("actions") != null) {
			...
			eventActions.setStateDelta(actionsMap.get("stateDelta") != null
					? new ConcurrentHashMap<>((Map<String, Object>) actionsMap.get("stateDelta"))
					: new ConcurrentHashMap<>());
			eventActions.setArtifactDelta(actionsMap.get("artifactDelta") != null
					? convertToArtifactDeltaMap(actionsMap.get("artifactDelta")) : new ConcurrentHashMap<>());
			eventActions.setTransferToAgent(
					actionsMap.get("transferAgent") != null ? (String) actionsMap.get("transferAgent") : null);
			eventActions.setEscalate(Optional.ofNullable(actionsMap.get("escalate")).map(value -> (Boolean) value));
			eventActions.setRequestedAuthConfigs(Optional.ofNullable(actionsMap.get("requestedAuthConfigs"))
		...
		Event event = Event.builder()
			...
			.content(Optional.ofNullable(apiEvent.get("content"))
			...
			.errorCode(Optional.ofNullable(apiEvent.get("errorCode")).map(value -> new FinishReason((String) value)))
			.errorMessage(Optional.ofNullable(apiEvent.get("errorMessage")).map(value -> (String) value))
		...
		if (apiEvent.get("eventMetadata") != null) {
			...
			if (rawGroundingMetadata != null) {
				groundingMetadata = objectMapper.convertValue(rawGroundingMetadata, GroundingMetadata.class);
			}

			event = event.toBuilder()
				.partial(Optional.ofNullable((Boolean) eventMetadata.get("partial")).orElse(false))
				.turnComplete(Optional.ofNullable((Boolean) eventMetadata.get("turnComplete")).orElse(false))
				.interrupted(Optional.ofNullable((Boolean) eventMetadata.get("interrupted")).orElse(false))
				.branch(Optional.ofNullable((String) eventMetadata.get("branch")))
				.groundingMetadata(groundingMetadata)
				.longRunningToolIds(longRunningToolIdsList != null ? new HashSet<>(longRunningToolIdsList) : null)
				.build();
		}
		return event;
	}
	...
	private static Instant convertToInstant(Object timestampObj) {
		if (timestampObj instanceof Map<?, ?> timestampMap) {
			return Instant.ofEpochSecond(((Number) timestampMap.get("seconds")).longValue(),
	...
	/**
	 * Converts a raw object from "artifactDelta" into a {@link ConcurrentMap} of
	 * {@link String} to {@link Part}.
	...
	private static ConcurrentMap<String, ConcurrentMap<String, Object>> asConcurrentMapOfConcurrentMaps(Object value) {
		return ((Map<String, Map<String, Object>>) value).entrySet()
			.stream()
