...
final class VertexAiClient {
	...
	VertexAiClient(String project, String location, Optional<GoogleCredentials> credentials,
	...
	Maybe<JsonNode> createSession(String reasoningEngineId, String userId, ConcurrentMap<String, Object> state) {
		...
		return Single.fromCallable(() -> objectMapper.writeValueAsString(sessionJsonMap))
			.flatMap(sessionJson -> performApiRequest("POST", "reasoningEngines/" + reasoningEngineId + "/sessions",
					sessionJson))
			.flatMapMaybe(apiResponse -> {
	...
	private Completable pollOperation(String operationId, int attempt) {
		if (attempt >= MAX_RETRY_ATTEMPTS) {
			return Completable.error(new TimeoutException("Operation " + operationId + " did not complete in time."));
		}
		return performApiRequest("GET", "operations/" + operationId, "").flatMapMaybe(VertexAiClient::getJsonResponse)
			.flatMapCompletable(lroJsonResponse -> {
				if (lroJsonResponse != null && lroJsonResponse.get("done") != null) {
					...
					return Completable.timer(1, SECONDS).andThen(pollOperation(operationId, attempt + 1));
				}
			});
	}

	Maybe<JsonNode> listSessions(String reasoningEngineId, String userId) {
		return performApiRequest("GET", "reasoningEngines/" + reasoningEngineId + "/sessions?filter=user_id=" + userId,
				"")
			.flatMapMaybe(VertexAiClient::getJsonResponse);
	}

	Maybe<JsonNode> listEvents(String reasoningEngineId, String sessionId) {
		return performApiRequest("GET", "reasoningEngines/" + reasoningEngineId + "/sessions/" + sessionId + "/events",
				"")
	...
	Maybe<JsonNode> getSession(String reasoningEngineId, String sessionId) {
		return performApiRequest("GET", "reasoningEngines/" + reasoningEngineId + "/sessions/" + sessionId, "")
			.flatMapMaybe(apiResponse -> getJsonResponse(apiResponse));
	}

	Completable deleteSession(String reasoningEngineId, String sessionId) {
		return performApiRequest("DELETE", "reasoningEngines/" + reasoningEngineId + "/sessions/" + sessionId, "")
			.doOnSuccess(ApiResponse::close)
			.ignoreElement();
	}

	Completable appendEvent(String reasoningEngineId, String sessionId, String eventJson) {
		return performApiRequest("POST",
				"reasoningEngines/" + reasoningEngineId + "/sessions/" + sessionId + ":appendEvent", eventJson)
	 ...
	 *
	 * <p>
	 * Note: The caller is responsible for closing the returned {@link ApiResponse}.
	 */
