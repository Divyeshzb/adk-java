...
/**
 * Utility class for capturing and reporting telemetry data within the ADK. This class
 * provides methods to trace various aspects of the agent's execution, including tool
 * calls, tool responses, LLM interactions, and data handling. It leverages OpenTelemetry
 * for tracing and logging for detailed information. These traces can then be exported
 * through the ADK Dev Server UI.
 */
public class Telemetry {
	...
	/**
	 * Sets the OpenTelemetry instance to be used for tracing. This is for testing
	 * purposes only.
	 */
	 ...
	 */
	public static void traceToolResponse(InvocationContext invocationContext, String eventId,
	 ...
	 * {@code GenerationConfig} is included as a whole. For other fields like
	 * {@code Content}, parts that cannot be easily serialized or are not needed for the
	 * trace (e.g., inlineData) are excluded.
	...
	private static Map<String, Object> buildLlmRequestForTrace(LlmRequest llmRequest) {
		...
		for (Content content : llmRequest.contents()) {
			ImmutableList<Part> filteredParts = content.parts()
	 ...
	 */
	public static void traceCallLlm(InvocationContext invocationContext, String eventId, LlmRequest llmRequest,
		...
		if (invocationContext.session() != null && invocationContext.session().id() != null) {
			...
			log.trace("traceCallLlm: InvocationContext session or session ID is null, cannot set"
		...
		try {
			...
			if (data != null) {
				for (Content content : data) {
					if (content != null) {
						dataList.add(JsonBaseModel.getMapper()
	 ...
	 *
	 * <p>
	 * This helper manages the OpenTelemetry Scope lifecycle for RxJava Flowables to
	 * ensure proper context propagation across async boundaries. The scope remains active
	 * from when the Flowable is returned through all operators until stream completion
	 * (onComplete, onError, or cancel).
	 *
	 * <p>
	 * <b>Why not try-with-resources?</b> RxJava Flowables execute lazily - operators run
	 * at subscription time, not at chain construction time. Using try-with-resources
	 * would close the scope before the Flowable subscribes, causing Context.current() to
	 * return ROOT in nested operations and breaking parent-child span relationships
	 * (fragmenting traces).
	 *
	 * <p>
	 * The scope is properly closed via doFinally when the stream terminates, ensuring no
	 ...
	 * @param flowableSupplier Supplier that creates the Flowable to execute with active
	 * scope
	 * @param <T> The type of items emitted by the Flowable
	 * @return Flowable with OpenTelemetry scope lifecycle management
	 */
	@SuppressWarnings("MustBeClosedChecker") // Scope lifecycle managed by RxJava
	// doFinally
	public static <T> Flowable<T> traceFlowable(Context spanContext, Span span,
