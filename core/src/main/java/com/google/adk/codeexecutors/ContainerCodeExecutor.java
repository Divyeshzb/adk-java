...
public class ContainerCodeExecutor extends BaseCodeExecutor {
	 ...
	 * @param image The tag of the predefined image or custom image to run on the
	 * container. Either dockerPath or image must be set.
	 * @param dockerPath The path to the directory containing the Dockerfile. If set,
	 * build the image from the dockerfile path instead of using the predefined image.
	...
	public ContainerCodeExecutor(Optional<String> baseUrl, Optional<String> image, Optional<String> dockerPath) {
		...
		if (baseUrl.isPresent()) {
			var config = DefaultDockerClientConfig.createDefaultConfigBuilder().withDockerHost(baseUrl.get()).build();
	...
	private void initContainer() {
		...
		if (dockerPath.isPresent()) {
			...
			// If a dockerPath is not provided, always pull the image to ensure it's
			// up-to-date.
		...
		var createContainerResponse = dockerClient.createContainerCmd(image).withTty(true).withAttachStdin(true).exec();
		dockerClient.startContainerCmd(createContainerResponse.getId()).exec();

		var containers = dockerClient.listContainersCmd().withShowAll(true).exec();
		this.container = containers.stream()
