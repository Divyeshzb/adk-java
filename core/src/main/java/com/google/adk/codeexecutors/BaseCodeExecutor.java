 ...
 * incorporate the execution results into the final response.
 */
public abstract class BaseCodeExecutor extends JsonBaseModel {

	/**
	 * If true, extract and process data files from the model request and attach them to
	 * the code executor.
	 *
	 * <p>
	...
	public List<List<String>> codeBlockDelimiters() {
		return ImmutableList.of(ImmutableList.of("```tool_code\n", "\n```"), ImmutableList.of("```python\n", "\n```"));
	}
	 ...
	 */
	public abstract CodeExecutionResult executeCode(InvocationContext invocationContext,
