...
public class IntegrationConnectorTool extends BaseTool {
	...
	private static final ImmutableList<String> EXCLUDE_FIELDS = ImmutableList.of("connectionName", "serviceName",
			"host", "entity", "operation", "action");

	private static final ImmutableList<String> OPTIONAL_FIELDS = ImmutableList.of("pageSize", "pageToken", "filter",
			"sortByColumns");

	/** Constructor for Application Integration Tool for integration */
	IntegrationConnectorTool(String openApiSpec, String pathUrl, String toolName, String toolDescription,
	...
	/**
	 * Constructor for Application Integration Tool with connection name, service name,
	 * host, entity, operation, and action
	 */
	IntegrationConnectorTool(String openApiSpec, String pathUrl, String toolName, String toolDescription,
			String connectionName, String serviceName, String host, String serviceAccountJson) {
		this(openApiSpec, pathUrl, toolName, toolDescription, connectionName, serviceName, host, serviceAccountJson,
				new DefaultHttpExecutor().createExecutor(serviceAccountJson));
	}

	IntegrationConnectorTool(String openApiSpec, String pathUrl, String toolName, String toolDescription,
			@Nullable String connectionName, @Nullable String serviceName, @Nullable String host,
			@Nullable String serviceAccountJson, HttpExecutor httpExecutor) {
	...
	private String executeIntegration(Map<String, Object> args) throws Exception {
		...
		if (response.statusCode() < 200 || response.statusCode() >= 300) {
			throw new Exception("Error executing integration. Status: " + response.statusCode() + " , Response: "
					+ response.body());
		}
	...
	private String getResolvedRequestSchemaByOperationId(String openApiSchemaString, String operationId)
			throws Exception {
		JsonNode topLevelNode = OBJECT_MAPPER.readTree(openApiSchemaString);
		JsonNode specNode = topLevelNode.path("openApiSpec");
		if (specNode.isMissingNode() || !specNode.isTextual()) {
		...
		JsonNode requestSchemaNode = operationNode.path("requestBody")
			.path("content")
		...
		if (resolvedSchema.isObject()) {
			...
			if (requiredNode.isArray()) {
				...
				List<String> fieldsToRemove = Streams.concat(OPTIONAL_FIELDS.stream(), EXCLUDE_FIELDS.stream())
					.toList();

				// To safely remove items from a list while iterating, we must use an
				// Iterator.
