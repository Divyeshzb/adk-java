 ...
 *
 * <ul>
 * <li>{@code connectionParams}: The connection parameters to the MCP server. Can be
 * either {@code
...
public class McpAsyncToolset implements BaseToolset {
	 ...
	 * @param toolFilter An Optional containing either a ToolPredicate or a List of tool
	 * names.
	 */
	public McpAsyncToolset(SseServerParameters connectionParams, ObjectMapper objectMapper,
	 ...
	 * names.
	 */
	public McpAsyncToolset(ServerParameters connectionParams, ObjectMapper objectMapper, Optional<Object> toolFilter) {
		Objects.requireNonNull(connectionParams);
		Objects.requireNonNull(objectMapper);
		this.objectMapper = objectMapper;
		this.mcpSessionManager = new McpSessionManager(connectionParams);
	 ...
	 * names.
	 */
	public McpAsyncToolset(McpSessionManager mcpSessionManager, ObjectMapper objectMapper,
			Optional<Object> toolFilter) {
	...
	public Flowable<BaseTool> getTools(ReadonlyContext readonlyContext) {
		return Maybe.defer(() -> Maybe.fromCompletionStage(this.initAndGetTools().toFuture()))
			...
			.map(tools -> tools.stream()
				.filter(tool -> isToolSelected(tool, toolFilter, Optional.ofNullable(readonlyContext)))
				.toList())
			.onErrorResumeNext(err -> {
				if (err instanceof McpToolsetException) {
					...
					return Single.error(new McpToolsetException.McpInitializationException(
							"Failed to reinitialize session during tool loading retry (unexpected" + " error).", err));
				}
			})
	...
	private Mono<List<McpAsyncTool>> initTools() {
		return Mono.defer(() -> {
			...
			return mcpSession.initialize()
				.doOnSuccess(initResult -> logger.debug("Initialize Client Result: {}", initResult))
				.thenReturn(mcpSession);
		})
			.flatMap(mcpSession -> mcpSession.listTools()
				.map(toolsResponse -> toolsResponse.tools()
					.stream()
					.map(tool -> new McpAsyncTool(tool, mcpSession, // move mcpSession to
							// McpAsyncTool
							this.mcpSessionManager, this.objectMapper))
					.toList()))
			.retryWhen(RetrySpec.from(retrySignal -> retrySignal.flatMap(signal -> {
				...
				if (err instanceof IllegalArgumentException) {
					...
					return Mono.error(new McpToolsetException.McpToolLoadingException(
							"Invalid argument encountered during tool loading.", err));
				}
				long totalRetries = signal.totalRetries();
				logger.error("Unexpected error during tool loading, retry attempt " + (totalRetries + 1), err);
				if (totalRetries < MAX_RETRIES) {
					...
					return Mono.error(new McpToolsetException.McpToolLoadingException(
							"Failed to load tools after multiple retries due to unexpected" + " error.", err));
				}
			})));
	}
	...
	public void close() {
		...
		if (tools != null) {
			tools.flatMapIterable(it -> it).flatMap(it -> it.mcpSession.closeGracefully().onErrorResume(e -> {
