 ...
 *
 * <p>
 * This class provides methods for retrieving OpenAPI spec for an integration or a
 * connection.
 */
public class IntegrationClient {
	...
	IntegrationClient(String project, String location, String integration, List<String> triggers, String connection,
			Map<String, List<String>> entityOperations, List<String> actions, HttpExecutor httpExecutor) {
	...
	private void validate() {
		...
		if (this.entityOperations != null) {
			Preconditions.checkArgument(!this.entityOperations.isEmpty(), "entityOperations map cannot be empty");
			for (Map.Entry<String, List<String>> entry : this.entityOperations.entrySet()) {
				...
				Preconditions.checkArgument(value != null, "Operations for entity '%s' cannot be null", key);
				for (String str : value) {
					Preconditions.checkArgument(str != null && !str.isEmpty(),
							"Operation for entity '%s' cannot be null or empty", key);
				}
			}
		}
		...
		if (this.actions != null) {
			...
			Preconditions.checkArgument(this.actions.stream().allMatch(Objects::nonNull),
					"Actions list cannot contain null values");
			Preconditions.checkArgument(this.actions.stream().noneMatch(String::isEmpty),
	...
	String generateOpenApiSpec() throws Exception {
		String url = String.format(
				"https://%s-integrations.googleapis.com/v1/projects/%s/locations/%s:generateOpenApiSpec", this.location,
				this.project, this.location);

		String jsonRequestBody = OBJECT_MAPPER.writeValueAsString(ImmutableMap.of("apiTriggerResources", ImmutableList
			.of(ImmutableMap.of("integrationResource", this.integration, "triggerId", Arrays.asList(this.triggers))),
				"fileFormat", "JSON"));
	...
	ObjectNode getOpenApiSpecForConnection(String toolName, String toolInstructions)
		...
		if (this.entityOperations != null) {
			for (Map.Entry<String, List<String>> entry : this.entityOperations.entrySet()) {
				...
				schemas.set("connectorInputPayload_" + entityLower,
				...
				for (String operation : operations) {
					...
					String path = String.format(
							...
							this.project, this.location, integrationName, integrationName, operationLower, entityLower);

					switch (operationLower) {
						case "create":
							paths.set(path, OBJECT_MAPPER.valueToTree(
									ConnectionsClient.createOperation(entityLower, toolName, toolInstructions)));
							schemas.set("create_" + entityLower + "_Request",
						...
						case "update":
							...
							schemas.set("update_" + entityLower + "_Request",
						...
						case "delete":
							...
							schemas.set("delete_" + entityLower + "_Request",
									OBJECT_MAPPER.valueToTree(ConnectionsClient.deleteOperationRequest()));
							break;
						case "list":
							paths.set(path, OBJECT_MAPPER.valueToTree(ConnectionsClient.listOperation(entityLower,
									jsonSchemaAsString, toolName, toolInstructions)));
							schemas.set("list_" + entityLower + "_Request",
									OBJECT_MAPPER.valueToTree(ConnectionsClient.listOperationRequest()));
							break;
						case "get":
							paths.set(path, OBJECT_MAPPER.valueToTree(ConnectionsClient.getOperation(entityLower,
									jsonSchemaAsString, toolName, toolInstructions)));
							schemas.set("get_" + entityLower + "_Request",
			...
			for (String action : this.actions) {
				ObjectNode actionDetails = OBJECT_MAPPER.valueToTree(connectionsClient.getActionSchema(action));
				...
				Map<String, Object> outputSchemaMap = OBJECT_MAPPER.treeToValue(outputSchemaNode, Map.class);

				if (Objects.equals(action, "ExecuteCustomQuery")) {
					schemas.set(actionDisplayName + "_Request",
				...
				paths.set(path, OBJECT_MAPPER.valueToTree(ConnectionsClient.getActionOperation(action, operation,
						actionDisplayName, toolName, toolInstructions)));
			}
		}
		else {
			throw new IllegalArgumentException(
