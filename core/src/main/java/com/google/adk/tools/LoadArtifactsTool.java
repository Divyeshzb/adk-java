 ...
 *
 * <p>
 * The declaration of this tool is consistent with the Python version. Refer to:
...
public final class LoadArtifactsTool extends BaseTool {
	...
	public Optional<FunctionDeclaration> declaration() {
		return Optional.of(FunctionDeclaration.builder()
			...
			.parameters(Schema.builder()
				.type("OBJECT")
				.properties(ImmutableMap.of("artifact_names",
	...
	public Single<Map<String, Object>> runAsync(Map<String, Object> args, ToolContext toolContext) {
		...
		List<String> artifactNames = (List<String>) args.getOrDefault("artifact_names", ImmutableList.of());
	...
	private Completable processLoadArtifactsFunctionCall(LlmRequest.Builder llmRequestBuilder,
		...
		return Iterables.getLast(currentContents)
			...
			.map(artifactNamesRaw -> {
				@SuppressWarnings("unchecked")
				List<String> artifactNamesToLoad = (List<String>) artifactNamesRaw;

				return Observable.fromIterable(artifactNamesToLoad)
					.flatMapCompletable(artifactName -> loadAndAppendIndividualArtifact(llmRequestBuilder, toolContext,
							artifactName));
			})
			.orElse(Completable.complete());
	}

	private Completable loadAndAppendIndividualArtifact(LlmRequest.Builder llmRequestBuilder, ToolContext toolContext,
			String artifactName) {

		return toolContext.loadArtifact(artifactName, Optional.empty())
			.flatMapCompletable(
					actualArtifact -> Completable.fromAction(() -> appendArtifactToLlmRequest(llmRequestBuilder,
							"Artifact " + artifactName + " is:", actualArtifact)));
	}

	private void appendArtifactToLlmRequest(LlmRequest.Builder llmRequestBuilder, String prefix, Part artifact) {
		llmRequestBuilder.contents(ImmutableList.<Content>builder()
