...
/**
 * A tool that injects (few-shot) examples into the outgoing LLM request as system
 * instructions.
 *
 * <p>
 ...
 *
 * <ul>
 * <li><b>examples</b>: Either a fully-qualified reference to a
 * {@link BaseExampleProvider} instance (e.g.,
 * <code>com.example.MyExamples.INSTANCE</code>) or a list of examples with fields
 * <code>input</code> and <code>output</code> (array of messages).
 * </ul>
 */
public final class ExampleTool extends BaseTool {
	...
	private ExampleTool(Builder builder) {
		super(isNullOrEmpty(builder.name) ? "example_tool" : builder.name,
				isNullOrEmpty(builder.description) ? "Adds few-shot examples to the request" : builder.description);
	...
	public Completable processLlmRequest(LlmRequest.Builder llmRequestBuilder, ToolContext toolContext) {
		...
		String query = toolContext.userContent()
	...
	public static ExampleTool fromConfig(ToolArgsConfig args, String configAbsPath) throws ConfigurationException {
		...
		try {
			...
			if (examplesArg instanceof List) {
				...
				for (Object o : rawList) {
					...
					ImmutableList<Content> outputs = outList.stream()
	...
	private static BaseExampleProvider resolveExampleProvider(String ref) throws ConfigurationException {
		...
		try {
			...
			throw new ConfigurationException("Field '" + fieldName + "' not found in class '" + className + "'", e);
		}
