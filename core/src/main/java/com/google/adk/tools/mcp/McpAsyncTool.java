 ...
 *
 * <p>
 * This wraps a MCP Tool interface and an active MCP Session. It invokes the MCP Tool
...
public final class McpAsyncTool extends AbstractMcpTool<McpAsyncClient> {
	 ...
	 */
	public McpAsyncTool(Tool mcpTool, McpAsyncClient mcpSession, McpSessionManager mcpSessionManager) {
	...
	public Single<Map<String, Object>> runAsync(Map<String, Object> args, ToolContext toolContext) {
		return Single
			.defer(() -> Maybe.fromCompletionStage(
					this.mcpSession.callTool(new CallToolRequest(this.name(), ImmutableMap.copyOf(args))).toFuture())
				.map(callResult -> wrapCallResult(this.objectMapper, this.name(), callResult))
				.switchIfEmpty(Single.fromCallable(() -> wrapCallResult(this.objectMapper, this.name(), null))))
			.retryWhen(errors -> errors.delay(100, MILLISECONDS)
				.take(3)
				.doOnNext(error -> logger.error("Retrying callTool due to: {}", error.getMessage(), error))
