...
public class FunctionTool extends BaseTool {
	...
	protected FunctionTool(@Nullable Object instance, Method func, boolean isLongRunning, boolean requireConfirmation) {
		super(func.isAnnotationPresent(Annotations.Schema.class)
		...
		this.func = func;
		this.funcDeclaration = FunctionCallingUtils.buildFunctionDeclaration(this.func,
				ImmutableList.of("toolContext", "inputStream"));
	...
	/**
	 * Returns true if the wrapped function returns a Flowable and can be used for
	 * streaming.
	 */
		...
		try {
			if (requireConfirmation) {
				if (toolContext.toolConfirmation().isEmpty()) {
					...
					return Single.just(ImmutableMap.of("error",
	...
	private Maybe<Map<String, Object>> call(Map<String, Object> args, ToolContext toolContext)
		...
		if (result == null) {
			...
			return ((Maybe<?>) result)
				.map(data -> OBJECT_MAPPER.convertValue(data, new TypeReference<Map<String, Object>>() {
				}));
		}
			...
			try {
				return Maybe.just(OBJECT_MAPPER.convertValue(result, new TypeReference<Map<String, Object>>() {
				}));
			}
			catch (IllegalArgumentException e) {
				// Conversion to map failed, in this case we follow
				// https://google.github.io/adk-docs/tools-custom/function-tools/#return-type
				// and return
				// the { "result": $result }
	...
	public Flowable<Map<String, Object>> callLive(Map<String, Object> args, ToolContext toolContext,
			InvocationContext invocationContext) throws IllegalAccessException, InvocationTargetException {
	...
	private Object[] buildArguments(Map<String, Object> args, ToolContext toolContext,
			@Nullable InvocationContext invocationContext) {
		Parameter[] parameters = func.getParameters();
		Object[] arguments = new Object[parameters.length];
		for (int i = 0; i < parameters.length; i++) {
			String paramName = parameters[i].isAnnotationPresent(Annotations.Schema.class)
					&& !parameters[i].getAnnotation(Annotations.Schema.class).name().isEmpty()
							? parameters[i].getAnnotation(Annotations.Schema.class).name() : parameters[i].getName();
			if ("toolContext".equals(paramName)) {
				arguments[i] = toolContext;
				continue;
			}
			if ("inputStream".equals(paramName)) {
				if (invocationContext != null && invocationContext.activeStreamingTools().containsKey(this.name())
			...
			if (!args.containsKey(paramName)) {
				if (schema != null && schema.optional()) {
					...
					throw new IllegalArgumentException(String
						.format("The parameter '%s' was not found in the arguments provided by the model.", paramName));
				}
			}
	...
	private static Class<?> getTypeClass(Type type, String paramName) {
		if (type instanceof Class) {
			...
			// Case 2: The argument is another parameterized type like Map<String,
			// Integer>
	...
	private static List<Object> createList(List<Object> values, Class<?> type) {
		...
		for (Object value : values) {
			if (cls == Integer.class || cls == Long.class || cls == Double.class || cls == Float.class
					|| cls == Boolean.class || cls == String.class) {
