 ...
 *
 * <p>
 * This class provides methods to fetch connection details, schemas for entities and
 * actions, and to generate OpenAPI specifications for creating tools based on these
 * connections.
 */
public class ConnectionsClient {
	 ...
	 */
	public ConnectionsClient(String project, String location, String connection, HttpExecutor httpExecutor,
	...
	public ConnectionDetails getConnectionDetails() throws IOException, InterruptedException {
		String url = String.format("%s/v1/projects/%s/locations/%s/connections/%s?view=BASIC", CONNECTOR_URL, project,
				location, connection);
	...
	public EntitySchemaAndOperations getEntitySchemaAndOperations(String entity)
		...
		Map<String, Object> responseData = (Map<String, Object>) operationResponse.getOrDefault("response",
				ImmutableMap.of());

		Map<String, Object> schema = (Map<String, Object>) responseData.getOrDefault("jsonSchema", ImmutableMap.of());
		List<String> operations = (List<String>) responseData.getOrDefault("operations", ImmutableList.of());
	...
	public ActionSchema getActionSchema(String action) throws IOException, InterruptedException {
		...
		actionSchema.inputSchema = (Map<String, Object>) responseData.getOrDefault("inputJsonSchema",
				ImmutableMap.of());
		actionSchema.outputSchema = (Map<String, Object>) responseData.getOrDefault("outputJsonSchema",
				ImmutableMap.of());
	...
	private HttpResponse<String> executeApiCall(String url) throws IOException, InterruptedException {
		...
		HttpResponse<String> response = httpExecutor.send(request, HttpResponse.BodyHandlers.ofString());

		if (response.statusCode() >= 400) {
			String body = response.body();
			if (response.statusCode() == 400 || response.statusCode() == 404) {
				throw new IllegalArgumentException(
						String.format("Invalid request. Please check the provided values of project(%s), location(%s),"
								+ " connection(%s). Error: %s", project, location, connection, body));
			}
	...
	public Map<String, Object> convertJsonSchemaToOpenApiSchema(Map<String, Object> jsonSchema) {
		...
		if (Objects.equals(openapiSchema.get("type"), "object") && jsonSchema.containsKey("properties")) {
			@SuppressWarnings("unchecked")
			Map<String, Map<String, Object>> properties = (Map<String, Map<String, Object>>) jsonSchema
				.get("properties");
	...
	public static ImmutableMap<String, Object> getConnectorBaseSpec() {
		return ImmutableMap.ofEntries(Map.entry("openapi", "3.0.1"),
				Map.entry("info",
						ImmutableMap.of("title", "ExecuteConnection", "description",
								"This tool can execute a query on connection", "version", "4")),
				Map.entry("servers", ImmutableList.of(ImmutableMap.of("url", "https://integrations.googleapis.com"))),
				Map.entry("security",
						ImmutableList.of(ImmutableMap.of("google_auth",
								ImmutableList.of("https://www.googleapis.com/auth/cloud-platform")))),
				Map.entry("paths", ImmutableMap.of()), Map.entry("components", ImmutableMap.ofEntries(
						Map.entry("schemas", ImmutableMap.ofEntries(
								Map.entry("operation", ImmutableMap.of("type", "string", "default", "LIST_ENTITIES",
										"description",
										"Operation to execute. Possible values are LIST_ENTITIES,"
												...
												+ " actions. and EXECUTE_QUERY in case of custom" + " queries.")),
								Map.entry("entityId",
										ImmutableMap.of("type", "string", "description", "Name of the entity")),
								Map.entry("connectorInputPayload", ImmutableMap.of("type", "object")),
								Map.entry("filterClause",
										ImmutableMap.of("type", "string", "default", "", "description",
												"WHERE clause in SQL query")),
								Map.entry("pageSize",
										ImmutableMap.of("type", "integer", "default", 50, "description",
												"Number of entities to return in the response")),
								Map.entry("pageToken",
										ImmutableMap.of("type", "string", "default", "", "description",
												"Page token to return the next page of entities")),
								Map.entry("connectionName",
										ImmutableMap.of("type", "string", "default", "", "description",
												"Connection resource name to run the query for")),
								Map.entry("serviceName",
										ImmutableMap.of("type", "string", "default", "", "description",
												"Service directory for the connection")),
								Map.entry("host",
										ImmutableMap.of("type", "string", "default", "", "description",
												"Host name incase of tls service directory")),
								Map.entry("entity",
										ImmutableMap.of("type", "string", "default", "Issues", "description",
												"Entity to run the query for")),
								Map.entry("action",
										ImmutableMap.of("type", "string", "default", "ExecuteCustomQuery",
												"description", "Action to run the query for")),
								Map.entry("query",
										ImmutableMap.of("type", "string", "default", "", "description",
												"Custom Query to execute on the connection")),
								Map.entry(
										"timeout",
										ImmutableMap.of("type", "integer", "default", 120, "description",
												"Timeout in seconds for execution of custom query")),
								Map.entry("sortByColumns",
										ImmutableMap.of("type", "array", "items", ImmutableMap.of("type", "string"),
												"default", ImmutableList.of(), "description",
								...
								Map.entry("execute-connector_Response", ImmutableMap.of("required",
										ImmutableList.of("connectorOutputPayload"), "type", "object", "properties",
										ImmutableMap.of("connectorOutputPayload",
												ImmutableMap.of("$ref", "#/components/schemas/connectorOutputPayload"),
												"nextPageToken",
												ImmutableMap.of("$ref", "#/components/schemas/nextPageToken")))))),
						Map.entry("securitySchemes", ImmutableMap
							.of("google_auth", ImmutableMap.of("type", "oauth2", "flows", ImmutableMap.of("implicit",
									ImmutableMap.of("authorizationUrl", "https://accounts.google.com/o/oauth2/auth",
											"scopes", ImmutableMap.of("https://www.googleapis.com/auth/cloud-platform",
													"Auth for google cloud services")))))))));
	}

	public static ImmutableMap<String, Object> getActionOperation(String action, String operation,
			String actionDisplayName, String toolName, String toolInstructions) {
		String description = "Use this tool to execute " + action;
		if (Objects.equals(operation, "EXECUTE_QUERY")) {
			description += " Use pageSize = 50 and timeout = 120 until user specifies a different value"
					+ " otherwise. If user provides a query in natural language, convert it to SQL query"
					+ " and then execute it using the tool.";
		}

		return ImmutableMap.of("post", ImmutableMap.ofEntries(Map.entry("summary", actionDisplayName),
				Map.entry("description", description + " " + toolInstructions),
				Map.entry("operationId", toolName + "_" + actionDisplayName), Map.entry("x-action", action),
				Map.entry("x-operation", operation),
				Map.entry("requestBody",
						ImmutableMap.of("content", ImmutableMap.of("application/json", ImmutableMap.of("schema",
								ImmutableMap.of("$ref",
										String.format("#/components/schemas/%s_Request", actionDisplayName)))))),
				Map.entry("responses",
						ImmutableMap.of("200", ImmutableMap.of("description", "Success response", "content",
								ImmutableMap.of("application/json", ImmutableMap.of("schema", ImmutableMap.of("$ref",
										String.format("#/components/schemas/%s_Response", actionDisplayName)))))))));
	}

	public static ImmutableMap<String, Object> listOperation(String entity, String schemaAsString, String toolName,
			String toolInstructions) {
		return ImmutableMap.of("post",
				ImmutableMap.ofEntries(Map.entry("summary", "List " + entity),
						Map.entry("description", String.format(
						...
						Map.entry("x-operation", "LIST_ENTITIES"), Map.entry("x-entity", entity),
						Map.entry("operationId", toolName + "_list_" + entity),
						Map.entry("requestBody", ImmutableMap.of("content", ImmutableMap.of("application/json",
								ImmutableMap.of("schema",
										ImmutableMap.of("$ref", "#/components/schemas/list_" + entity + "_Request"))))),
						Map.entry("responses",
								ImmutableMap.of("200", ImmutableMap.of("description", "Success response", "content",
										ImmutableMap.of("application/json",
												ImmutableMap.of("schema", ImmutableMap.of("description",
														String.format("Returns a list of %s of json schema: %s", entity,
																schemaAsString),
														"$ref",
														"#/components/schemas/execute-connector_Response"))))))));
	}

	public static ImmutableMap<String, Object> getOperation(String entity, String schemaAsString, String toolName,
			String toolInstructions) {
		return ImmutableMap.of("post",
				ImmutableMap.ofEntries(Map.entry("summary", "Get " + entity),
						Map.entry("description",
								String.format("Returns the details of the %s. %s", entity, toolInstructions)),
						Map.entry("operationId", toolName + "_get_" + entity), Map.entry("x-operation", "GET_ENTITY"),
						...
						Map.entry("responses", ImmutableMap.of("200", ImmutableMap.of("description", "Success response",
								"content",
								ImmutableMap.of("application/json", ImmutableMap.of("schema",
										ImmutableMap.of("description",
												String.format("Returns %s of json schema: %s", entity, schemaAsString),
												"$ref", "#/components/schemas/execute-connector_Response"))))))));
	}

	public static ImmutableMap<String, Object> createOperation(String entity, String toolName,
			String toolInstructions) {
		return ImmutableMap.of("post", ImmutableMap.ofEntries(Map.entry("summary", "Creates a new " + entity),
				Map.entry("description", String.format("Creates a new %s. %s", entity, toolInstructions)),
				Map.entry("x-operation", "CREATE_ENTITY"), Map.entry("x-entity", entity),
				Map.entry("operationId", toolName + "_create_" + entity),
				Map.entry("requestBody", ImmutableMap.of("content", ImmutableMap.of("application/json",
						ImmutableMap.of("schema",
								ImmutableMap.of("$ref", "#/components/schemas/create_" + entity + "_Request"))))),
				Map.entry("responses", ImmutableMap.of("200",
						ImmutableMap.of("description", "Success response", "content", ImmutableMap
							.of("application/json", ImmutableMap.of("schema",
									ImmutableMap.of("$ref", "#/components/schemas/execute-connector_Response"))))))));
	}

	public static ImmutableMap<String, Object> updateOperation(String entity, String toolName,
			String toolInstructions) {
		return ImmutableMap.of("post", ImmutableMap.ofEntries(Map.entry("summary", "Updates the " + entity),
				Map.entry("description", String.format("Updates the %s. %s", entity, toolInstructions)),
				Map.entry("x-operation", "UPDATE_ENTITY"), Map.entry("x-entity", entity),
				Map.entry("operationId", toolName + "_update_" + entity),
				Map.entry("requestBody", ImmutableMap.of("content", ImmutableMap.of("application/json",
						ImmutableMap.of("schema",
								ImmutableMap.of("$ref", "#/components/schemas/update_" + entity + "_Request"))))),
				Map.entry("responses", ImmutableMap.of("200",
						ImmutableMap.of("description", "Success response", "content", ImmutableMap
							.of("application/json", ImmutableMap.of("schema",
									ImmutableMap.of("$ref", "#/components/schemas/execute-connector_Response"))))))));
	...
	public static ImmutableMap<String, Object> deleteOperation(String entity, String toolName,
			String toolInstructions) {
		return ImmutableMap.of("post", ImmutableMap.ofEntries(Map.entry("summary", "Delete the " + entity),
				Map.entry("description", String.format("Deletes the %s. %s", entity, toolInstructions)),
				Map.entry("x-operation", "DELETE_ENTITY"), Map.entry("x-entity", entity),
				Map.entry("operationId", toolName + "_delete_" + entity),
				Map.entry("requestBody", ImmutableMap.of("content", ImmutableMap.of("application/json",
						ImmutableMap.of("schema",
								ImmutableMap.of("$ref", "#/components/schemas/delete_" + entity + "_Request"))))),
				Map.entry("responses", ImmutableMap.of("200",
						ImmutableMap.of("description", "Success response", "content", ImmutableMap
							.of("application/json", ImmutableMap.of("schema",
									ImmutableMap.of("$ref", "#/components/schemas/execute-connector_Response"))))))));
	...
	public static ImmutableMap<String, Object> createOperationRequest(String entity) {
		return ImmutableMap
			.of("type", "object", "required",
					ImmutableList
						.of("connectorInputPayload", "operation", "connectionName", "serviceName", "host", "entity"),
					"properties",
					ImmutableMap
						.ofEntries(
								Map.entry("connectorInputPayload",
										ImmutableMap.of("$ref",
												"#/components/schemas/connectorInputPayload_" + entity)),
								Map.entry("operation", ImmutableMap.of("$ref", "#/components/schemas/operation")),
								Map.entry("connectionName",
										ImmutableMap.of("$ref", "#/components/schemas/connectionName")),
	...
	public static ImmutableMap<String, Object> updateOperationRequest(String entity) {
		return ImmutableMap.of("type", "object", "required", ImmutableList
			.of("connectorInputPayload", "entityId", "operation", "connectionName", "serviceName", "host", "entity"),
				"properties",
				ImmutableMap.ofEntries(
						...
						Map.entry("operation", ImmutableMap.of("$ref", "#/components/schemas/operation")),
						Map.entry("connectionName", ImmutableMap.of("$ref", "#/components/schemas/connectionName")),
						Map.entry("serviceName", ImmutableMap.of("$ref", "#/components/schemas/serviceName")),
						Map.entry("host", ImmutableMap.of("$ref", "#/components/schemas/host")),
						Map.entry("entity", ImmutableMap.of("$ref", "#/components/schemas/entity")),
	...
	public static ImmutableMap<String, Object> getOperationRequest() {
		return ImmutableMap.of("type", "object", "required",
				ImmutableList.of("entityId", "operation", "connectionName", "serviceName", "host", "entity"),
				"properties",
				ImmutableMap.ofEntries(Map.entry("entityId", ImmutableMap.of("$ref", "#/components/schemas/entityId")),
						Map.entry("operation", ImmutableMap.of("$ref", "#/components/schemas/operation")),
						Map.entry("connectionName", ImmutableMap.of("$ref", "#/components/schemas/connectionName")),
						Map.entry("serviceName", ImmutableMap.of("$ref", "#/components/schemas/serviceName")),
						Map.entry("host", ImmutableMap.of("$ref", "#/components/schemas/host")),
	...
	public static ImmutableMap<String, Object> listOperationRequest() {
		return ImmutableMap.of("type", "object", "required",
				ImmutableList.of("operation", "connectionName", "serviceName", "host", "entity"), "properties",
				ImmutableMap.ofEntries(
						Map.entry("filterClause", ImmutableMap.of("$ref", "#/components/schemas/filterClause")),
						...
						Map.entry("host", ImmutableMap.of("$ref", "#/components/schemas/host")),
						Map.entry("entity", ImmutableMap.of("$ref", "#/components/schemas/entity")),
						Map.entry("sortByColumns", ImmutableMap.of("$ref", "#/components/schemas/sortByColumns"))));
	}

	public static ImmutableMap<String, Object> actionRequest(String action) {
		return ImmutableMap.of("type", "object", "required",
				ImmutableList
					.of("operation", "connectionName", "serviceName", "host", "action", "connectorInputPayload"),
				"properties",
				ImmutableMap.ofEntries(
						Map.entry("operation", ImmutableMap.of("$ref", "#/components/schemas/operation")),
						Map.entry("connectionName", ImmutableMap.of("$ref", "#/components/schemas/connectionName")),
	...
	public static ImmutableMap<String, Object> actionResponse(String action) {
		return ImmutableMap.of("type", "object", "properties", ImmutableMap.of("connectorOutputPayload",
	...
	public static ImmutableMap<String, Object> executeCustomQueryRequest() {
		return ImmutableMap.of("type", "object", "required",
				ImmutableList
					.of("operation", "connectionName", "serviceName", "host", "action", "query", "timeout", "pageSize"),
				"properties",
				ImmutableMap.ofEntries(
						Map.entry("operation", ImmutableMap.of("$ref", "#/components/schemas/operation")),
						Map.entry("connectionName", ImmutableMap.of("$ref", "#/components/schemas/connectionName")),
