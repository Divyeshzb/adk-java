...
public final class GoogleSearchTool extends BaseTool {
	...
	public Completable processLlmRequest(LlmRequest.Builder llmRequestBuilder, ToolContext toolContext) {
		...
		if (model != null && model.startsWith("gemini-1")) {
			...
			updatedToolsBuilder
				.add(Tool.builder().googleSearchRetrieval(GoogleSearchRetrieval.builder().build()).build());
			...
			return Completable
				.error(new IllegalArgumentException("Google search tool is not supported for model " + model));
		}
