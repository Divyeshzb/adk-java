...
/**
 * The default builder for creating MCP client transports. Supports StdioClientTransport
 * based on {@link ServerParameters}, HttpClientSseClientTransport based on
 * {@link SseServerParameters}, and HttpClientStreamableHttpTransport based on
 * {@link StreamableHttpServerParameters}.
 */
	...
	public McpClientTransport build(Object connectionParams) {
		if (connectionParams instanceof ServerParameters serverParameters) {
			...
			return HttpClientSseClientTransport.builder(sseServerParams.url())
				.sseEndpoint(sseServerParams.sseEndpoint() == null ? "sse" : sseServerParams.sseEndpoint())
				.customizeRequest(builder -> Optional.ofNullable(sseServerParams.headers())
					.map(ImmutableMap::entrySet)
					.stream()
					.flatMap(Collection::stream)
					.forEach(entry -> builder.header(entry.getKey(),
			...
			throw new IllegalArgumentException(
					"DefaultMcpTransportBuilder supports only ServerParameters, SseServerParameters, or"
							+ " StreamableHttpServerParameters, but got " + connectionParams.getClass().getName());
		}
	}
