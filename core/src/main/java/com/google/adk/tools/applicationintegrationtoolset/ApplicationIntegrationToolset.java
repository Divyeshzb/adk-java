...
public class ApplicationIntegrationToolset implements BaseToolset {
	 ...
	 * location="us-central1", integration=null, triggers=null,
	 * connection="test-connection", entityOperations=ImmutableMap.of("Entity1",
	 * ImmutableList.of("LIST", "GET", "UPDATE")), "Entity2", ImmutableList.of()),
	 * actions=ImmutableList.of("ExecuteCustomQuery"), serviceAccountJson="{....}",
	 * toolNamePrefix="test-tool", toolInstructions="This tool is used to list, get and
	 ...
	 * @param serviceAccountJson(Optional) The service account configuration as a
	 * dictionary. Required if not using default service credential. Used for fetching the
	 * Application Integration or Integration Connector resource.
	 * @param toolNamePrefix(Optional) The tool name prefix.
	 * @param toolInstructions(Optional) The tool instructions.
	 */
	public ApplicationIntegrationToolset(String project, String location, String integration, List<String> triggers,
			String connection, Map<String, List<String>> entityOperations, List<String> actions,
			String serviceAccountJson, String toolNamePrefix, String toolInstructions) {
		this(project, location, integration, triggers, connection, entityOperations, actions, serviceAccountJson,
				toolNamePrefix, toolInstructions, new DefaultHttpExecutor().createExecutor(serviceAccountJson));
	}

	ApplicationIntegrationToolset(String project, String location, String integration, List<String> triggers,
			String connection, Map<String, List<String>> entityOperations, List<String> actions,
			String serviceAccountJson, String toolNamePrefix, String toolInstructions, HttpExecutor httpExecutor) {
	...
	private List<BaseTool> getAllTools() throws Exception {
		...
		if (!isNullOrEmpty(this.integration)) {
			IntegrationClient integrationClient = new IntegrationClient(this.project, this.location, this.integration,
					this.triggers, null, null, null, this.httpExecutor);
			...
			for (String pathUrl : pathUrls) {
				...
				if (toolName != null) {
					tools.add(new IntegrationConnectorTool(openApiSchemaString, pathUrl, toolName, toolInstructions,
							null, null, null, this.serviceAccountJson, this.httpExecutor));
				}
			}
		}
		else if (!isNullOrEmpty(this.connection) && (this.entityOperations != null || this.actions != null)) {
			IntegrationClient integrationClient = new IntegrationClient(this.project, this.location, null, null,
					this.connection, this.entityOperations, this.actions, this.httpExecutor);
			ObjectNode parentOpenApiSpec = OBJECT_MAPPER.createObjectNode();
			ObjectNode openApiSpec = integrationClient.getOpenApiSpecForConnection(toolNamePrefix, toolInstructions);
			...
			for (String pathUrl : pathUrls) {
				...
				if (!isNullOrEmpty(toolName)) {
					ConnectionsClient connectionsClient = new ConnectionsClient(this.project, this.location,
							this.connection, this.httpExecutor, OBJECT_MAPPER);
					ConnectionsClient.ConnectionDetails connectionDetails = connectionsClient.getConnectionDetails();

					tools.add(new IntegrationConnectorTool(openApiSchemaString, pathUrl, toolName, "",
							connectionDetails.name, connectionDetails.serviceName, connectionDetails.host,
							this.serviceAccountJson, this.httpExecutor));
				}
			}
		}
		else {
			throw new IllegalArgumentException("Invalid request, Either integration or (connection and"
