 ...
 *
 * <p>
 * This class allows requests to be routed through an Apigee proxy. The model string
 * format allows for specifying the provider (Gemini or Vertex AI), API version, and model
 * ID.
 */
public class ApigeeLlm extends BaseLlm {
	...
	private ApigeeLlm(String modelName, String proxyUrl, Map<String, String> customHeaders) {
		...
		if (isNullOrEmpty(effectiveProxyUrl)) {
			throw new IllegalArgumentException(
					"Apigee proxy URL is not set and not found in the environment variable" + " APIGEE_PROXY_URL.");
		}

		// Build the Client
		HttpOptions.Builder httpOptionsBuilder = HttpOptions.builder()
			.baseUrl(effectiveProxyUrl)
		...
		if (customHeaders != null) {
			httpOptionsBuilder.headers(ImmutableMap.<String, String>builder()
	...
	/**
	 * Returns the genai {@link com.google.genai.Client} instance for making API calls for
	 * testing purposes.
	...
	private static boolean isVertexAiModel(String model) {
		...
		// Otherwise, it is Vertex AI if either the user has explicitly set the model
		// string to be
		// "apigee/vertex_ai/" or the GOOGLE_GENAI_USE_VERTEXAI environment variable is
		// set.
		return !model.startsWith("apigee/gemini/")
				&& (model.startsWith("apigee/vertex_ai/") || isEnvEnabled(GOOGLE_GENAI_USE_VERTEXAI_ENV_VARIABLE_NAME));
	}
	...
	public static class Builder {
		...
		/**
		 * Sets the model string. The model string specifies the LLM provider (e.g.,
		 * Vertex AI, Gemini), API version, and the model ID.
		 *
		 * <p>
		 * <b>Format:</b> {@code apigee/[<provider>/][<version>/]<model_id>}
		 *
		 * <p>
		 * <b>Components:</b>
		 *
		 * <ul>
		 * <li><b>{@code provider}</b> (optional): {@code vertex_ai} or {@code gemini}. If
		 * omitted, behavior depends on the {@code GOOGLE_GENAI_USE_VERTEXAI} environment
		 * variable. If that is not set to {@code TRUE} or {@code 1}, it defaults to
		 * {@code gemini}.
		 * <li><b>{@code version}</b> (optional): The API version (e.g., {@code v1},
		 * {@code v1beta}). If omitted, the default version for the provider is used.
