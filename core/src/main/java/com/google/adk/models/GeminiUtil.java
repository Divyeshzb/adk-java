...
public final class GeminiUtil {
	...
	public static final String CONTINUE_OUTPUT_MESSAGE = "Continue output. DO NOT look at this line. ONLY look at the content before this line and"
			+ " system instruction.";
	 ...
	 *
	 * <p>
	 * This method can optionally sanitize the request and ensures that the last content
	 * part is from the user to prompt a model response.
	 * @param llmRequest The original {@link LlmRequest}.
	 * @param sanitize Whether to sanitize the request to be compatible with the Gemini
	 * API backend.
	 ...
	 * <p>
	 * This method can optionally sanitize the request and ensures that the last content
	 * part is from the user to prompt a model response. It also strips out any parts
	 * marked as "thoughts".
	 * @param llmRequest The original {@link LlmRequest}.
	 * @param sanitize Whether to sanitize the request to be compatible with the Gemini
	 * API backend.
	 ...
	 * Required as there are some parameters that if included in the request will raise a
	 * runtime error if sent to the wrong backend (e.g. image names only work on Vertex
	 * AI).
	...
	public static LlmRequest sanitizeRequestForGeminiApi(LlmRequest llmRequest) {
		...
		llmRequest.config()
			...
			.ifPresent(config -> requestBuilder.config(config.toBuilder().labels(ImmutableMap.of()).build()));
		...
		ImmutableList<Content> updatedContents = llmRequest.contents().stream().map(content -> {
			if (content.parts().isEmpty() || content.parts().get().isEmpty()) {
				return content;
			}

			ImmutableList<Part> updatedParts = content.parts().get().stream().map(part -> {
	...
	/**
	 * Ensures that the content is conducive to prompting a model response by ensuring the
	 * last content part is from the user.
	 *
	 * <p>
	 * If the list is empty or the last message is not from the user, a new "user" content
	 * part with a {@link #CONTINUE_OUTPUT_MESSAGE} is appended to the list. This is
	 * necessary to prompt the model to generate a response.
	 * @param contents The original list of {@link Content}.
	 * @return A list of {@link Content} where the last element is guaranteed to be from
	 * the "user".
	 */
	static List<Content> ensureModelResponse(List<Content> contents) {
		...
		if (contents.isEmpty() || !Ascii.equalsIgnoreCase(Iterables.getLast(contents).role().orElse(""), "user")) {
	 ...
	 * We flush if current response is not a text continuation (e.g., no content, no
	 * parts, or the first part is not inline_data, meaning it's something else or just
	 * empty, thereby warranting a flush of preceding text).
	...
	public static boolean shouldEmitAccumulatedText(LlmResponse currentLlmResponse) {
		...
		// This means we return true if content, parts, or the first part's inlineData is
		// empty.
		return currentLlmResponse.content()
			.flatMap(Content::parts)
			.filter(parts -> !parts.isEmpty())
			.map(parts -> parts.get(0))
	...
	public static List<Content> stripThoughts(List<Content> originalContents) {
		return originalContents.stream().map(content -> {
			ImmutableList<Part> nonThoughtParts = content.parts()
