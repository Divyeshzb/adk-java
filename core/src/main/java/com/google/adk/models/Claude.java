 ...
 * connections are not currently supported for Claude.
 */
public class Claude extends BaseLlm {
	 ...
	 * @param modelName The name of the Claude model to use (e.g.,
	...
	public Flowable<LlmResponse> generateContent(LlmRequest llmRequest, boolean stream) {
		...
		List<MessageParam> messages = llmRequest.contents()
			.stream()
		...
		ToolChoice toolChoice = llmRequest.tools().isEmpty() ? null
		...
		if (configOpt.isPresent()) {
			...
			if (systemInstructionOpt.isPresent()) {
				String extractedSystemText = systemInstructionOpt.get()
					.parts()
	...
	private MessageParam contentToAnthropicMessageParam(Content content) {
		return MessageParam.builder()
			...
			.contentOfBlockParams(content.parts()
				.orElse(ImmutableList.of())
				.stream()
	...
	private ContentBlockParam partToAnthropicMessageBlock(Part part) {
		if (part.text().isPresent()) {
			...
			return ContentBlockParam.ofToolUse(ToolUseBlockParam.builder()
				...
				.input(com.anthropic.core.JsonValue.from(part.functionCall().get().args().orElse(ImmutableMap.of())))
				.build());
		}
	...
	private void updateTypeString(Map<String, Object> valueDict) {
		...
		if (valueDict.containsKey("items")) {
			...
			if (valueDict.get("items") instanceof Map && ((Map) valueDict.get("items")).containsKey("properties")) {
	...
	private Tool functionDeclarationToAnthropicTool(FunctionDeclaration functionDeclaration) {
		...
		if (functionDeclaration.parameters().isPresent()
			...
			functionDeclaration.parameters().get().properties().get().forEach((key, schema) -> {
				ObjectMapper objectMapper = new ObjectMapper();
				objectMapper.registerModule(new Jdk8Module());
				Map<String, Object> schemaMap = objectMapper.convertValue(schema,
		...
		return Tool.builder()
			...
			.inputSchema(Tool.InputSchema.builder().properties(com.anthropic.core.JsonValue.from(properties)).build())
			.build();
	}
		...
		if (block.isText()) {
			...
			return Part.builder()
				.functionCall(FunctionCall.builder()
					...
					.args(block.asToolUse()._input().convert(new TypeReference<Map<String, Object>>() {
					}))
					.build())
				.build();
		}
