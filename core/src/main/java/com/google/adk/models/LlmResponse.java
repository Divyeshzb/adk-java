...
public abstract class LlmResponse extends JsonBaseModel {
	 ...
	 * @return An {@link Content} of the first {@link Candidate} in the
	 * {@link GenerateContentResponse} if the response contains at least one candidate.,
	 * or an empty optional if no candidates are present in the response.
	 */
	@JsonProperty("content")
	public abstract Optional<Content> content();

	/**
	 * Returns the grounding metadata of the first candidate in the response, if
	 * available.
	...
	/**
	 * Indicates that LLM was interrupted when generating the content. Usually it's due to
	 * user interruption during a bidi streaming.
	 */
	...
	public abstract static class Builder {
		...
		@JsonProperty("usageMetadata")
		public abstract Builder usageMetadata(@Nullable GenerateContentResponseUsageMetadata usageMetadata);

		public abstract Builder usageMetadata(Optional<GenerateContentResponseUsageMetadata> usageMetadata);
		...
		public final Builder response(GenerateContentResponse response) {
			...
			if (candidatesOpt.isPresent() && !candidatesOpt.get().isEmpty()) {
				...
				Optional<GenerateContentResponsePromptFeedback> promptFeedbackOpt = response.promptFeedback();
