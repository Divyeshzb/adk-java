...
public final class GcsArtifactService implements BaseArtifactService {
	...
	public Single<Integer> saveArtifact(String appName, String userId, String sessionId, String filename,
		...
		return listVersions(appName, userId, sessionId, filename)
			...
			.map(nextVersion -> {
				...
				try {
					byte[] dataToSave = artifact.inlineData()
						.get()
						.data()
						.orElseThrow(() -> new IllegalArgumentException("Saveable artifact data must be non-empty."));
	...
	public Maybe<Part> loadArtifact(String appName, String userId, String sessionId, String filename,
		...
		return version.map(Maybe::just)
			.orElseGet(() -> listVersions(appName, userId, sessionId, filename)
				.flatMapMaybe(versions -> versions.isEmpty() ? Maybe.empty() : Maybe.just(max(versions))))
			.flatMap(versionToLoad -> {
	...
	public Single<ListArtifactsResponse> listArtifactKeys(String appName, String userId, String sessionId) {
		...
		try {
			for (Blob blob : storageClient.list(bucketName, BlobListOption.prefix(sessionPrefix)).iterateAll()) {
		...
		try {
			for (Blob blob : storageClient.list(bucketName, BlobListOption.prefix(userPrefix)).iterateAll()) {
				List<String> parts = Splitter.on('/').splitToList(blob.getName());
		...
		return Single.just(ListArtifactsResponse.builder().filenames(ImmutableList.sortedCopyOf(filenames)).build());
	}
	...
	public Completable deleteArtifact(String appName, String userId, String sessionId, String filename) {
		ImmutableList<Integer> versions = listVersions(appName, userId, sessionId, filename).blockingGet();
	...
	public Single<ImmutableList<Integer>> listVersions(String appName, String userId, String sessionId,
		...
		try {
			for (Blob blob : storageClient.list(bucketName, BlobListOption.prefix(prefix)).iterateAll()) {
				...
				int versionDelimiterIndex = name.lastIndexOf('/'); // immediately before
				// the version number
