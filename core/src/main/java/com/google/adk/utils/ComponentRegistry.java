 ...
 *
 * <p>
 * This class provides a base registry with common ADK components and is designed to be
 * extended by users who want to add their own pre-wired entries. The registry is fully
 * thread-safe and supports storing any type of object.
 *
 * <p>
...
public class ComponentRegistry {
	...
	public void register(String name, Object value) {
		...
		if (previous != null) {
			logger.info("Overriding existing registration for name: {} (was: {}, now: {})", name,
					previous.getClass().getSimpleName(), value.getClass().getSimpleName());
		}
		else {
			logger.debug("Registered new object of type {} with name: {}", value.getClass().getSimpleName(), name);
		}
	}
	 ...
	 * @return an Optional containing the object if found and castable to the specified
	 * type, or an empty Optional otherwise
	 */
	public <T> Optional<T> get(String name, Class<T> type) {
		return get(name).filter(value -> {
			if (type.isInstance(value)) {
				return true;
			}
			else {
				logger.info("Object with name '{}' is of type {} but expected type {}", name,
	...
	/**
	 * Updates the global singleton instance with a new ComponentRegistry. This is useful
	 * for replacing the default registry with a custom one when the server starts.
	 *
	 * <p>
	 * This method is thread-safe and ensures that all threads see the updated instance
	 * atomically.
	 ...
	 *
	 * <p>
	 * This method looks up an agent in the ComponentRegistry by the given key. The
	 * registry should have been pre-populated with all available agents during
	 * initialization.
	 *
	 * <p>
	...
	public static Class<? extends BaseAgent> resolveAgentClass(String agentClassName) {
		...
		return agentClass.orElseThrow(() -> new IllegalArgumentException(
	...
	/**
	 * Resolves an instance from the registry by name, attempting various prefixes if the
	 * name is not fully qualified.
	 * @param name The name of the instance to resolve.
	 * @param type The expected type of the instance.
	 * @param <T> The type parameter.
	 * @return An Optional containing the instance if found and castable to the specified
	...
	private static <T> Optional<T> resolveInstance(String name, String adkPackage, Class<T> type) {
		if (isNullOrEmpty(name)) {
			...
			return getInstance().get(name, type)
				.or(() -> getInstance().get(String.format("com.google.adk.%s.%s", adkPackage, name), type))
	 ...
	 *
	 * <p>
	 * This method follows the same pattern as {@code resolveToolClass} but for
	 * BaseToolset implementations. It first checks the registry, then attempts direct
	 * class loading if the name contains a dot (indicating a fully qualified class name).
	...
	public static Optional<Class<? extends BaseToolset>> resolveToolsetClass(String toolsetClassName) {
		if (isNullOrEmpty(toolsetClassName)) {
			...
			return getType(toolsetClassName, BaseToolset.class).or(() -> loadToolsetClass(toolsetClassName));
		}
		else {
			// First try the simple name, then with google.adk.tools prefix (consistent
			// with
			// resolveToolClass)
	...
	public Set<String> getToolNamesWithPrefix(String prefix) {
		return registry.keySet().stream().filter(name -> name.startsWith(prefix)).collect(toImmutableSet());
	}
	 ...
	 * @return An Optional containing the Class if found and assignable to the specified
	...
	private static <T> Optional<Class<? extends T>> getType(String name, Class<T> type) {
		return getInstance().get(name, Class.class).filter(type::isAssignableFrom).map(clazz -> clazz.asSubclass(type));
	}
