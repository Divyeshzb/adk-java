...
public class RequestConfirmationLlmRequestProcessor implements RequestProcessor {
	...
	public Single<RequestProcessor.RequestProcessingResult> processRequest(InvocationContext invocationContext,
		...
		ImmutableMap<String, ToolConfirmation> requestConfirmationFunctionResponses = filterRequestConfirmationFunctionResponses(
				events);
		...
		for (ImmutableList<FunctionCall> functionCalls : events.stream()
			.map(Event::functionCalls)
			.filter(fc -> !fc.isEmpty())
			.collect(toImmutableList())) {

			ImmutableMap<String, FunctionCall> toolsToResumeWithArgs = filterToolsToResumeWithArgs(functionCalls,
					requestConfirmationFunctionResponses);
			ImmutableMap<String, ToolConfirmation> toolsToResumeWithConfirmation = toolsToResumeWithArgs.keySet()
				.stream()
				.filter(id -> events.stream()
			...
			return assembleEvent(invocationContext, toolsToResumeWithArgs.values(), toolsToResumeWithConfirmation)
	...
	private Maybe<Event> assembleEvent(InvocationContext invocationContext, Collection<FunctionCall> functionCalls,
		...
		var functionCallEvent = Event.builder()
		...
		return Functions.handleFunctionCalls(invocationContext, functionCallEvent, toolsBuilder.buildOrThrow(),
				toolConfirmations);
	}
	...
	private ImmutableMap<String, ToolConfirmation> filterRequestConfirmationFunctionResponses(List<Event> events) {
		return events.stream()
			...
			.filter(functionResponse -> Objects.equals(functionResponse.name().orElse(null),
	...
	private Optional<Map.Entry<String, ToolConfirmation>> maybeCreateToolConfirmationEntry(
		...
		if (responseMap.size() != 1 || !responseMap.containsKey("response")) {
			return Optional.of(Map.entry(functionResponse.id().get(),
					objectMapper.convertValue(responseMap, ToolConfirmation.class)));
		}

		try {
			return Optional.of(Map.entry(functionResponse.id().get(),
					objectMapper.readValue((String) responseMap.get("response"), ToolConfirmation.class)));
		}
	...
	private ImmutableMap<String, FunctionCall> filterToolsToResumeWithArgs(ImmutableList<FunctionCall> functionCalls,
		...
		return functionCalls.stream()
			...
			.filter(fc -> Objects.equals(fc.name().orElse(null), REQUEST_CONFIRMATION_FUNCTION_CALL_NAME))
			.filter(fc -> fc.args().orElse(ImmutableMap.of()).containsKey("originalFunctionCall"))
			.collect(toImmutableMap(fc -> fc.id().get(),
					fc -> objectMapper.convertValue(fc.args().get().get("originalFunctionCall"), FunctionCall.class)));
	}
