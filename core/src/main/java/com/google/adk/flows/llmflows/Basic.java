...
public final class Basic implements RequestProcessor {
	...
	public Single<RequestProcessor.RequestProcessingResult> processRequest(InvocationContext context,
		...
		String modelName = agent.resolvedModel().model().isPresent() ? agent.resolvedModel().model().get().model()
				: agent.resolvedModel().modelName().get();

		LiveConnectConfig.Builder liveConnectConfigBuilder = LiveConnectConfig.builder()
			.responseModalities(context.runConfig().responseModalities());
		Optional.ofNullable(context.runConfig().speechConfig()).ifPresent(liveConnectConfigBuilder::speechConfig);
