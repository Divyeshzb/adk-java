...
public final class Functions {
	...
	public static void populateClientFunctionCallId(Event modelResponseEvent) {
		...
		for (Part part : originalParts) {
			if (part.functionCall().isPresent()) {
				...
				if (functionCall.id().isEmpty() || functionCall.id().get().isEmpty()) {
					FunctionCall updatedFunctionCall = functionCall.toBuilder()
		...
		if (modified) {
			String role = originalContent.role()
	...
	public static Maybe<Event> handleFunctionCalls(InvocationContext invocationContext, Event functionCallEvent,
	...
	public static Maybe<Event> handleFunctionCalls(InvocationContext invocationContext, Event functionCallEvent,
			Map<String, BaseTool> tools, Map<String, ToolConfirmation> toolConfirmations) {
		...
		Function<FunctionCall, Maybe<Event>> functionCallMapper = functionCall -> {
			BaseTool tool = tools.get(functionCall.name().get());
			ToolContext toolContext = ToolContext.builder(invocationContext)
			...
			Maybe<Map<String, Object>> maybeFunctionResult = maybeInvokeBeforeToolCall(invocationContext, tool,
					functionArgs, toolContext)
				.switchIfEmpty(Maybe.defer(() -> callTool(tool, functionArgs, toolContext)));

			return maybeFunctionResult.map(Optional::of)
				.defaultIfEmpty(Optional.empty())
				.onErrorResumeNext(t -> invocationContext.pluginManager()
				...
				.flatMapMaybe(optionalInitialResult -> {
					...
					Maybe<Map<String, Object>> afterToolResultMaybe = maybeInvokeAfterToolCall(invocationContext, tool,
							functionArgs, toolContext, initialFunctionResult);

					return afterToolResultMaybe.map(Optional::of)
						.defaultIfEmpty(Optional.ofNullable(initialFunctionResult))
						.flatMapMaybe(finalOptionalResult -> {
							Map<String, Object> finalFunctionResult = finalOptionalResult.orElse(null);
							if (tool.longRunning() && finalFunctionResult == null) {
								return Maybe.empty();
							}
							Event functionResponseEvent = buildResponseEvent(tool, finalFunctionResult, toolContext,
									invocationContext);
		...
		if (invocationContext.runConfig().toolExecutionMode() == ToolExecutionMode.SEQUENTIAL) {
			functionResponseEventsFlowable = Flowable.fromIterable(functionCalls).concatMapMaybe(functionCallMapper);
		}
		else {
			functionResponseEventsFlowable = Flowable.fromIterable(functionCalls).flatMapMaybe(functionCallMapper);
		}
	...
	/**
	 * Handles function calls in a live/streaming context, supporting background execution
	 * and stream termination.
	 */
	public static Maybe<Event> handleFunctionCallsLive(InvocationContext invocationContext, Event functionCallEvent,
			Map<String, BaseTool> tools) {
		ImmutableList<FunctionCall> functionCalls = functionCallEvent.functionCalls();
		...
		Function<FunctionCall, Maybe<Event>> functionCallMapper = functionCall -> {
			...
			Maybe<Map<String, Object>> maybeFunctionResult = maybeInvokeBeforeToolCall(invocationContext, tool,
				...
				.switchIfEmpty(Maybe.defer(
						() -> processFunctionLive(invocationContext, tool, toolContext, functionCall, functionArgs)));
		...
		if (invocationContext.runConfig().toolExecutionMode() == ToolExecutionMode.SEQUENTIAL) {
			responseEventsFlowable = Flowable.fromIterable(functionCalls).concatMapMaybe(functionCallMapper);

		}
		else {
			responseEventsFlowable = Flowable.fromIterable(functionCalls).flatMapMaybe(functionCallMapper);
		}
	...
	/**
	 * Processes a single function call in a live context. Manages starting, stopping, and
	 * running tools.
	 */
	private static Maybe<Map<String, Object>> processFunctionLive(InvocationContext invocationContext, BaseTool tool,
			ToolContext toolContext, FunctionCall functionCall, Map<String, Object> args) {
		...
		if (functionCall.name().get().equals("stopStreaming") && args.containsKey("functionName")) {
			...
			if (activeTool != null) {
				...
				return Maybe
					.just(ImmutableMap.of("status", "Successfully stopped streaming function " + functionNameToStop));
			}
			else {
				logger.warn("No active streaming function named {} found to stop", functionNameToStop);
				return Maybe
					.just(ImmutableMap.of("status", "No active streaming function named " + functionNameToStop));
			}
		}

		// Case 2: Handle a streaming-capable tool (FunctionTool with Flowable return
		// type)
		if (tool instanceof FunctionTool functionTool) {
			if (functionTool.isStreaming()) {
				try {
					Flowable<Map<String, Object>> toolOutputStream = functionTool.callLive(args, toolContext,
							invocationContext);

					// Subscribe to the tool's output to process results in the
					// background.
					Disposable subscription = toolOutputStream.subscribe(result -> {
						String resultText = "Function " + tool.name() + " returned: " + result;
						Content updateContent = Content.builder().role("user").parts(Part.fromText(resultText)).build();
					...
					ActiveStreamingTool activeTool = invocationContext.activeStreamingTools()
					...
					return Maybe.just(ImmutableMap.of("status",
	...
	public static Set<String> getLongRunningFunctionCalls(List<FunctionCall> functionCalls,
			Map<String, BaseTool> tools) {
	...
	private static Maybe<Map<String, Object>> maybeInvokeBeforeToolCall(InvocationContext invocationContext,
			BaseTool tool, Map<String, Object> functionArgs, ToolContext toolContext) {
		if (invocationContext.agent() instanceof LlmAgent) {
			LlmAgent agent = (LlmAgent) invocationContext.agent();

			Maybe<Map<String, Object>> pluginResult = invocationContext.pluginManager()
				.runBeforeToolCallback(tool, functionArgs, toolContext);
			...
			Maybe<Map<String, Object>> callbackResult = Maybe.defer(() -> Flowable.fromIterable(callbacks)
				.concatMapMaybe(callback -> callback.call(invocationContext, tool, functionArgs, toolContext))
				.firstElement());
	...
	private static Maybe<Map<String, Object>> maybeInvokeAfterToolCall(InvocationContext invocationContext,
			BaseTool tool, Map<String, Object> functionArgs, ToolContext toolContext,
		...
		if (invocationContext.agent() instanceof LlmAgent) {
			...
			Maybe<Map<String, Object>> callbackResult = Maybe.defer(() -> Flowable.fromIterable(callbacks)
				.concatMapMaybe(
						callback -> callback.call(invocationContext, tool, functionArgs, toolContext, functionResult))
				.firstElement());
	...
	private static Maybe<Map<String, Object>> callTool(BaseTool tool, Map<String, Object> args,
		...
		return Maybe.defer(() -> {
			...
			try (Scope scope = span.makeCurrent()) {
				...
				return tool.runAsync(args, toolContext).toMaybe().doOnError(span::recordException).doFinally(span::end);
			}
	...
	private static Event buildResponseEvent(BaseTool tool, Map<String, Object> response, ToolContext toolContext,
		...
		try (Scope scope = span.makeCurrent()) {
			...
			Part partFunctionResponse = Part.builder()
			...
			Event event = Event.builder()
				...
				.content(Optional
					.of(Content.builder().role("user").parts(Collections.singletonList(partFunctionResponse)).build()))
	 ...
	 */
	public static Optional<Event> generateRequestConfirmationEvent(InvocationContext invocationContext,
			Event functionCallEvent, Event functionResponseEvent) {
		...
		ImmutableMap<String, FunctionCall> functionCallsById = functionCallEvent.functionCalls()
			.stream()
			.filter(fc -> fc.id().isPresent())
			.collect(toImmutableMap(fc -> fc.id().get(), fc -> fc));

		for (Map.Entry<String, ToolConfirmation> entry : functionResponseEvent.actions()
			...
			FunctionCall requestConfirmationFunctionCall = FunctionCall.builder()
				.name(REQUEST_CONFIRMATION_FUNCTION_CALL_NAME)
				.args(ImmutableMap.of("originalFunctionCall", functionCallsById.get(entry.getKey()), "toolConfirmation",
						entry.getValue()))
				.id(generateClientFunctionCallId())
				.build();
