...
public class LlmAgent extends BaseAgent {
	...
	/**
	 * Enum to define if contents of previous events should be included in requests to the
	 * underlying LLM.
	 */
	...
	// TODO: Remove exampleProvider field - examples should only be provided via
	// ExampleTool
	...
	protected LlmAgent(Builder builder) {
		super(builder.name, builder.description, builder.subAgents, builder.beforeAgentCallback,
		...
		this.globalInstruction = builder.globalInstruction == null ? new Instruction.Static("")
				: builder.globalInstruction;
	...
	public static class Builder extends BaseAgent.Builder<Builder> {
		...
		// TODO: Remove these example provider methods and only use ExampleTool for
		// providing examples.
		// Direct example methods should be deprecated in favor of using ExampleTool
		// consistently.
		@CanIgnoreReturnValue
			...
			if (beforeModelCallback == null) {
				...
				for (BeforeModelCallbackBase callback : beforeModelCallback) {
					if (callback instanceof BeforeModelCallback beforeModelCallbackInstance) {
						...
						builder.add((BeforeModelCallback) (callbackContext, llmRequestBuilder) -> Maybe
							.fromOptional(beforeModelCallbackSyncInstance.call(callbackContext, llmRequestBuilder)));
					}
					else {
						logger.warn("Invalid beforeModelCallback callback type: %s. Ignoring this callback.",
		...
		public Builder beforeModelCallbackSync(BeforeModelCallbackSync beforeModelCallbackSync) {
			this.beforeModelCallback = ImmutableList.of((callbackContext, llmRequestBuilder) -> Maybe
				.fromOptional(beforeModelCallbackSync.call(callbackContext, llmRequestBuilder)));
			return this;
		}
			...
			if (afterModelCallback == null) {
				...
				for (AfterModelCallbackBase callback : afterModelCallback) {
					if (callback instanceof AfterModelCallback afterModelCallbackInstance) {
						...
						builder.add((AfterModelCallback) (callbackContext, llmResponse) -> Maybe
							.fromOptional(afterModelCallbackSyncInstance.call(callbackContext, llmResponse)));
					}
					else {
						logger.warn("Invalid afterModelCallback callback type: %s. Ignoring this callback.",
								callback.getClass().getName());
					}
				}
		...
		public Builder afterModelCallbackSync(AfterModelCallbackSync afterModelCallbackSync) {
			this.afterModelCallback = ImmutableList.of((callbackContext, llmResponse) -> Maybe
				.fromOptional(afterModelCallbackSync.call(callbackContext, llmResponse)));
			return this;
		}

		@CanIgnoreReturnValue
		public Builder beforeAgentCallbackSync(BeforeAgentCallbackSync beforeAgentCallbackSync) {
			this.beforeAgentCallback = ImmutableList
				.of((callbackContext) -> Maybe.fromOptional(beforeAgentCallbackSync.call(callbackContext)));
			return this;
		}

		@CanIgnoreReturnValue
		public Builder afterAgentCallbackSync(AfterAgentCallbackSync afterAgentCallbackSync) {
			this.afterAgentCallback = ImmutableList
				.of((callbackContext) -> Maybe.fromOptional(afterAgentCallbackSync.call(callbackContext)));
			return this;
		}
		...
		@CanIgnoreReturnValue
		public Builder beforeToolCallback(@Nullable List<? extends BeforeToolCallbackBase> beforeToolCallbacks) {
			if (beforeToolCallbacks == null) {
				...
				for (BeforeToolCallbackBase callback : beforeToolCallbacks) {
					if (callback instanceof BeforeToolCallback beforeToolCallbackInstance) {
						...
						builder.add((invocationContext, baseTool, input, toolContext) -> Maybe.fromOptional(
								beforeToolCallbackSyncInstance.call(invocationContext, baseTool, input, toolContext)));
					}
					else {
						logger.warn("Invalid beforeToolCallback callback type: {}. Ignoring this callback.",
								callback.getClass().getName());
					}
				}
		...
		public Builder beforeToolCallbackSync(BeforeToolCallbackSync beforeToolCallbackSync) {
			this.beforeToolCallback = ImmutableList.of((invocationContext, baseTool, input, toolContext) -> Maybe
				.fromOptional(beforeToolCallbackSync.call(invocationContext, baseTool, input, toolContext)));
			return this;
		}
			...
			if (afterToolCallbacks == null) {
				...
				for (AfterToolCallbackBase callback : afterToolCallbacks) {
					if (callback instanceof AfterToolCallback afterToolCallbackInstance) {
						...
						builder.add((invocationContext, baseTool, input, toolContext, response) -> Maybe
							.fromOptional(afterToolCallbackSyncInstance.call(invocationContext, baseTool, input,
									toolContext, response)));
					}
					else {
						logger.warn("Invalid afterToolCallback callback type: {}. Ignoring this callback.",
								callback.getClass().getName());
					}
				}
		...
		public Builder afterToolCallbackSync(AfterToolCallbackSync afterToolCallbackSync) {
			this.afterToolCallback = ImmutableList
				.of((invocationContext, baseTool, input, toolContext, response) -> Maybe.fromOptional(
						afterToolCallbackSync.call(invocationContext, baseTool, input, toolContext, response)));
			return this;
		}
		...
		protected void validate() {
			...
			if (this.outputSchema != null) {
				if (!this.disallowTransferToParent || !this.disallowTransferToPeers) {
					System.err.println("Warning: Invalid config for agent " + this.name
				...
				if (this.subAgents != null && !this.subAgents.isEmpty()) {
					throw new IllegalArgumentException("Invalid config for agent " + this.name
	...
	private void maybeSaveOutputToState(Event event) {
		if (outputKey().isPresent() && event.finalResponse() && event.content().isPresent()) {
			...
			Object output;
			String rawResult = event.content()
			...
			if (outputSchema.isPresent()) {
				try {
					...
					logger.error("LlmAgent output for outputKey '{}' was not valid JSON, despite an outputSchema being"
							+ " present. Saving raw output to state.", outputKey().get(), e);
					output = rawResult;
				}
				catch (IllegalArgumentException e) {
					logger.error("LlmAgent output for outputKey '{}' did not match the outputSchema. Saving raw output"
							+ " to state.", outputKey().get(), e);
					output = rawResult;
				}
			}
			else {
	...
	protected Flowable<Event> runAsyncImpl(InvocationContext invocationContext) {
		return llmFlow.run(invocationContext).concatMap(event -> {
	...
	/**
	 * Constructs the text instruction for this agent based on the {@link #instruction}
	 * field. Also returns a boolean indicating that state injection should be bypassed
	 * when the instruction is constructed with an {@link Instruction.Provider}.
	 *
	 * <p>
	 * This method is only for use by Agent Development Kit.
	 * @param context The context to retrieve the session state.
	 * @return The resolved instruction as a {@link Single} wrapped Map.Entry. The key is
	 * the instruction string and the value is a boolean indicating if state injection
	 * should be bypassed.
	 */
	 ...
	 * {@link #globalInstruction} field. Also returns a boolean indicating that state
	 * injection should be bypassed when the instruction is constructed with an
	 * {@link Instruction.Provider}.
	 *
	 * <p>
	 * This method is only for use by Agent Development Kit.
	 * @param context The context to retrieve the session state.
	 * @return The resolved global instruction as a {@link Single} wrapped Map.Entry. The
	 * key is the instruction string and the value is a boolean indicating if state
	 * injection should be bypassed.
	 */
	 ...
	 * This method is only for use by Agent Development Kit.
	 * @param context The context to retrieve the session state.
	 * @return The resolved list of tools as a {@link Single} wrapped list of
	 * {@link BaseTool}.
	 */
	public Flowable<BaseTool> canonicalTools(Optional<ReadonlyContext> context) {
		...
		for (Object toolOrToolset : toolsUnion) {
			if (toolOrToolset instanceof BaseTool baseTool) {
				...
				throw new IllegalArgumentException(
						"Object in tools list is not of a supported type: " + toolOrToolset.getClass().getName());
			}
		}
	...
	/**
	 * Resolves the model for this agent, checking first if it is defined locally, then
	 * searching through ancestors.
	 *
	 * <p>
	 * This method is only for use by Agent Development Kit.
	 ...
	 * @param configAbsPath The absolute path to the agent config file. This is needed for
	 * resolving relative paths for e.g. tools and subagents.
	 * @return the configured LlmAgent
	 * @throws ConfigurationException if the configuration is invalid
	 */
	public static LlmAgent fromConfig(LlmAgentConfig config, String configAbsPath) throws ConfigurationException {
		...
		logger.info("Successfully created LlmAgent: {} with {} subagents", agent.name(),
				agent.subAgents() != null ? agent.subAgents().size() : 0);

		return agent;
	}

	private static void setCallbacksFromConfig(LlmAgentConfig config, Builder builder) throws ConfigurationException {
		ConfigAgentUtils.resolveAndSetCallback(config.beforeModelCallbacks(), Callbacks.BeforeModelCallbackBase.class,
				"before_model_callback", builder::beforeModelCallback);
		ConfigAgentUtils.resolveAndSetCallback(config.afterModelCallbacks(), Callbacks.AfterModelCallbackBase.class,
				"after_model_callback", builder::afterModelCallback);
		ConfigAgentUtils.resolveAndSetCallback(config.beforeToolCallbacks(), Callbacks.BeforeToolCallbackBase.class,
				"before_tool_callback", builder::beforeToolCallback);
		ConfigAgentUtils.resolveAndSetCallback(config.afterToolCallbacks(), Callbacks.AfterToolCallbackBase.class,
				"after_tool_callback", builder::afterToolCallback);
	}
