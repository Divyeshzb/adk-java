...
final class ToolResolver {
	...
	/**
	 * Resolves a list of tool configurations into both {@link BaseTool} and
	 * {@link BaseToolset} instances.
	 *
	 * <p>
	 * This method is only for use by Agent Development Kit.
	 * @param toolConfigs The list of tool configurations to resolve.
	 * @param configAbsPath The absolute path to the agent config file currently being
	 * processed. This path can be used to resolve relative paths for tool configurations,
	 * if necessary.
	 * @return An immutable list of resolved {@link BaseTool} and {@link BaseToolset}
	 * instances.
	 * @throws ConfigurationException if any tool configuration is invalid (e.g., missing
	 * name), if a tool cannot be found by its name or class, or if tool instantiation
	 * fails.
	 */
	static ImmutableList<Object> resolveToolsAndToolsets(List<ToolConfig> toolConfigs, String configAbsPath)
	 ...
	 *
	 * <p>
	 * It first checks the {@link ComponentRegistry} for a registered tool instance. If
	 * not found, and the name looks like a fully qualified Java name referencing a static
	 * field (e.g., "com.google.mytools.MyToolClass.INSTANCE"), it attempts to resolve it
	 * via reflection using {@link #resolveInstanceViaReflection(String)}.
	 * @param toolName The name of the tool or a static field reference (e.g., "myTool",
	 * "com.google.mytools.MyToolClass.INSTANCE").
	 * @return The resolved tool instance, or {@code null} if the tool is not found in the
	 ...
	 *
	 * <p>
	 * It first checks the {@link ComponentRegistry} for a registered toolset instance. If
	 * not found, it attempts to resolve the toolset via reflection if the name looks like
	 * a Java qualified name (e.g., "com.google.mytools.MyToolsetClass.INSTANCE").
	 * @param toolsetName The name of the toolset to resolve (could be simple name or full
	 * qualified "com.google.mytools.MyToolsetClass.INSTANCE").
	 * @return The resolved toolset instance, or {@code null} if the toolset is not found
	 * in the registry and cannot be resolved via reflection.
	 */
	@Nullable
	 ...
	 *
	 * <p>
	 * It attempts to resolve the toolset class using the ComponentRegistry, then
	 * instantiates it using the fromConfig method if available.
	 * @param className The name of the toolset class to instantiate.
	 * @param args Configuration arguments for toolset creation.
	 * @return The instantiated toolset instance, or {@code null} if the class cannot be
	 * found or is not a toolset.
	 * @throws ConfigurationException if toolset instantiation fails.
	 */
	@Nullable
	static BaseToolset resolveToolsetFromClass(String className, ToolArgsConfig args) throws ConfigurationException {
		ComponentRegistry registry = ComponentRegistry.getInstance();
		...
		try {
			Method fromConfigMethod = toolsetClass.getMethod("fromConfig", ToolConfig.class, String.class);
			...
			throw new ConfigurationException("Access denied to fromConfig method on class " + className, e);
		}
		catch (InvocationTargetException e) {
			logger.error("Error during fromConfig method invocation on toolset class {}", className, e.getCause());
			throw new ConfigurationException("Error during toolset creation from class " + className, e.getCause());
		}
		catch (RuntimeException e) {
			logger.error("Unexpected error calling fromConfig on toolset class {}", className, e);
			throw new ConfigurationException("Unexpected error creating toolset from class " + className, e);
		}
	 ...
	 * @param toolsetName The toolset name in format
	 * "com.example.MyToolsetClass.INSTANCE".
	 * @return The resolved toolset instance, or {@code null} if not found or not a
	 * BaseToolset.
	 ...
	 *
	 * <p>
	 * It attempts to load the class specified by {@code className}. If {@code args} are
	 * provided and non-empty, it looks for a static factory method
	 * {@code fromConfig(ToolArgsConfig)} on the class to instantiate the tool. If
	 * {@code args} are null or empty, it looks for a default constructor.
	 * @param className The fully qualified name of the tool class to instantiate.
	 * @param args Optional configuration arguments for tool creation. If provided, the
	 * class must implement a static {@code fromConfig(ToolArgsConfig)} factory method. If
	 * null or empty, the class must have a default constructor.
	 * @return The instantiated tool instance, or {@code null} if the class cannot be
	 * found or loaded.
	 * @throws ConfigurationException if {@code args} are provided but no
	 * {@code fromConfig} method exists, if {@code args} are not provided but no default
	 * constructor exists, or if instantiation via the factory method or constructor
	 * fails.
	 */
	@Nullable
	static BaseTool resolveToolFromClass(String className, ToolArgsConfig args) throws ConfigurationException {
		ComponentRegistry registry = ComponentRegistry.getInstance();
		...
		try {
			...
			throw new ConfigurationException(
					"Class " + className + " does not have a default constructor and no args were provided.", e);
		}
		catch (Exception e) {
	...
	/**
	 * Checks if a string appears to be a Java fully qualified name, such as
	 * "com.google.adk.MyClass" or "com.google.adk.MyClass.MY_FIELD".
	 *
	 * <p>
	 * It verifies that the name contains at least one dot ('.') and consists of
	 * characters valid for Java identifiers and package names.
	 * @param name The string to check.
	 * @return {@code true} if the string matches the pattern of a Java qualified name,
	...
	/**
	 * Resolves a {@link BaseTool} instance by attempting to access a public static field
	 * via reflection.
	 *
	 * <p>
	 * This method expects {@code toolName} to be in the format
	 * "com.google.package.ClassName.STATIC_FIELD_NAME", where "STATIC_FIELD_NAME" is the
	 * name of a public static field in "com.google.package.ClassName" that holds a
	 * {@link BaseTool} instance.
	 * @param toolName The fully qualified name of a static field holding a tool instance.
	 * @return The {@link BaseTool} instance, or {@code null} if {@code toolName} is not
	 * in the expected format, or if the field is not found, not static, or not of type
	 * {@link BaseTool}.
	 * @throws Exception if the class specified in {@code toolName} cannot be loaded, or
	 * if there is a security manager preventing reflection, or if accessing the field
	 * causes an exception.
	 */
	@Nullable
